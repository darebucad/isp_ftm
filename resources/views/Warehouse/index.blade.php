    ...
    <div class="x_panel">
        ...
        <div class="x_title">
          ...
          <ul class="nav navbar-right panel_toolbox">
            <li>
                <input type="button" class="btn btn-primary" value="New Warehouse" onclick="window.location.href='/Warehouse/Add'" />
            </li>
          </ul>
...
@section('js')
  ...
  <script>
    ...
    $(document).ready(function(){
      table = $("#tblWarehouse").DataTable({
        "pageLength": 30,
        "processing": true,
        "serverSide": true,
        "ajax": "{{route('api.getWarehouse')}}",
        "columns":[
          {
            "width": "20%",
            "data":"Name",
          ...
          },
          {
            ...
            render: function ( data, type, row ) {
              return '<button type="button" class="btn btn-default" onclick="editWarehouse(\''+data.Id+'\')">Edit</button>'
      ...
      $("#btnDelete").click(function(){
        ...
        $.ajax({
                 headers: {
                  ...
                  async:false,
                  type:'GET',
                  url:'/Warehouse/Delete/' + catId,
                  success:function(data){
                      if(data.errors != undefined && data.errors.length > 0){
                        showErrorMessage(data.errors);
                      }else{
                        toastr.success('Warehouse deleted','success', {timeOut: 3000});
                        $("#btnCancel").click();
                        table.ajax.reload();
                      }
                  },
